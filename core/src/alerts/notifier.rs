...
///
/// Pass `email_client` when a Loops email client is available; email-type
/// channels are silently skipped when `None`.
pub async fn send_notification(
    channel: &NotificationChannel,
    notification: &AlertNotification,
    email_client: Option<&crate::email::LoopsClient>,
) -> Result<(), anyhow::Error> {
    if !channel.enabled {
    ...
    if channel.channel_type == "email" {
        let recipient = channel.config["email"]
            .as_str()
            .ok_or_else(|| anyhow::anyhow!("Email channel missing recipient email address"))?;
        let client = email_client.ok_or_else(|| {
            anyhow::anyhow!("Email channel configured but no email client available")
        })?;

        let severity = if notification.is_missing {
            "warning"
        } else {
            "critical"
        };
        let message = notification
            .annotations
            .get("summary")
            .or_else(|| notification.annotations.get("description"))
            .cloned()
            .unwrap_or_else(|| {
                format!(
                    "{}: {}",
        ...
        client
            .send_alert(
                recipient,
                crate::email::AlertVars {
                    alert_name: notification.rule_name.clone(),
        ...
        info!(
            "Sent email notification for alert {}",
    ...
    let (url, payload, auth_header) = match channel.channel_type.as_str() {
        "slack" => {
            ...
            let bot_token = crate::crypto::SecretEncryptor::from_env()
                .and_then(|enc| {
