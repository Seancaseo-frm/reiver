...
//!
//! Vertex AI exposes an OpenAI-compatible `/chat/completions` endpoint. This
//! adapter is a thin wrapper over `OpenAiProvider` that:
//!
//! 1. Namespaces model identifiers with a `vertex/` prefix so they don't
//!    collide with other providers.
//! 2. Strips the `vertex/` prefix from the model name before forwarding.
...
pub struct VertexAiProvider(OpenAiProvider);
...
impl LlmProvider for VertexAiProvider {
    ...
    fn supports_model(&self, model: &str) -> bool {
        model.starts_with("vertex/")
    }

    #[tracing::instrument(
        name = "provider.vertex_ai.chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            message_count = request.messages.len(),
            gen_ai.provider.name = "vertex-ai",
    ...
    #[tracing::instrument(
        name = "provider.vertex_ai.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "vertex-ai",
