...
//!
//! DeepInfra hosts open-source models at low cost behind an
...
//! | Env var | Description |
//! |---|---|
//! | `GATEWAY_DEEPINFRA_BASE_URL` | API endpoint (default: `https://api.deepinfra.com/v1/openai`) |
//! | `GATEWAY_TIMEOUT_DEEPINFRA_SECONDS` | HTTP timeout (default: 120) |
...
impl LlmProvider for DeepInfraProvider {
    ...
    fn supports_model(&self, model: &str) -> bool {
        model.starts_with("deepinfra/")
    }

    #[tracing::instrument(
        name = "provider.deepinfra.chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            message_count = request.messages.len(),
            gen_ai.provider.name = "deepinfra",
    ...
    #[tracing::instrument(
        name = "provider.deepinfra.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "deepinfra",
