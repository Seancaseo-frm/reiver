...
//!
//! OvhCloud exposes an OpenAI-compatible `/chat/completions` endpoint. This
//! adapter is a thin wrapper over `OpenAiProvider` that:
//!
//! 1. Namespaces model identifiers with a `ovhcloud/` prefix so they don't
//!    collide with other providers.
//! 2. Strips the `ovhcloud/` prefix from the model name before forwarding.
...
impl OvhCloudProvider {
    ...
    fn strip_prefix(model: &str) -> &str {
        model.strip_prefix("ovhcloud/").unwrap_or(model)
    }
}
...
impl LlmProvider for OvhCloudProvider {
    ...
    fn supports_model(&self, model: &str) -> bool {
        model.starts_with("ovhcloud/")
    }

    #[tracing::instrument(
        name = "provider.ovhcloud.chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            message_count = request.messages.len(),
            gen_ai.provider.name = "ovhcloud",
    ...
    #[tracing::instrument(
        name = "provider.ovhcloud.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "ovhcloud",
