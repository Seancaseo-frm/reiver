//! Alibaba Cloud (Qwen via DashScope) provider adapter.
//!
//! Alibaba's DashScope exposes Qwen models behind an OpenAI-compatible
...
//! | Env var | Description |
//! |---|---|
//! | `GATEWAY_ALIBABA_BASE_URL` | API endpoint (default: `https://dashscope-intl.aliyuncs.com/compatible-mode/v1`) |
//! | `GATEWAY_TIMEOUT_ALIBABA_SECONDS` | HTTP timeout (default: 120) |
...
impl AlibabaProvider {
    ...
    fn strip_prefix(model: &str) -> &str {
        model.strip_prefix("qwen/").unwrap_or(model)
    }
}
...
impl LlmProvider for AlibabaProvider {
    ...
    #[tracing::instrument(
        name = "provider.alibaba.chat_completion",
    ...
    #[tracing::instrument(
        name = "provider.alibaba.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "qwen",
