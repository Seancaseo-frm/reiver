...
//!
//! SambaNova exposes an OpenAI-compatible `/chat/completions` endpoint. This
//! adapter is a thin wrapper over `OpenAiProvider` that:
//!
//! 1. Namespaces model identifiers with a `sambanova/` prefix so they don't
//!    collide with other providers.
//! 2. Strips the `sambanova/` prefix from the model name before forwarding.
...
pub struct SambaNovaProvider(OpenAiProvider);
...
impl SambaNovaProvider {
    ...
    fn strip_prefix(model: &str) -> &str {
        model.strip_prefix("sambanova/").unwrap_or(model)
    }
}
...
impl LlmProvider for SambaNovaProvider {
    ...
    fn supports_model(&self, model: &str) -> bool {
        model.starts_with("sambanova/")
    }

    #[tracing::instrument(
        name = "provider.sambanova.chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            message_count = request.messages.len(),
            gen_ai.provider.name = "sambanova",
    ...
    #[tracing::instrument(
        name = "provider.sambanova.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "sambanova",
