...
//!
//! Cerebras offers ultra-fast inference on custom Wafer-Scale Engine silicon
...
//! | Env var | Description |
//! |---|---|
//! | `GATEWAY_CEREBRAS_BASE_URL` | API endpoint (default: `https://api.cerebras.ai/v1`) |
//! | `GATEWAY_TIMEOUT_CEREBRAS_SECONDS` | HTTP timeout (default: 120) |
...
impl CerebrasProvider {
    ...
    fn strip_prefix(model: &str) -> &str {
        model.strip_prefix("cerebras/").unwrap_or(model)
    }
}
...
impl LlmProvider for CerebrasProvider {
    ...
    fn supports_model(&self, model: &str) -> bool {
        model.starts_with("cerebras/")
    }

    #[tracing::instrument(
        name = "provider.cerebras.chat_completion",
    ...
    #[tracing::instrument(
        name = "provider.cerebras.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "cerebras",
