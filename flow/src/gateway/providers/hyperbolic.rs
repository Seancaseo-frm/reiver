...
//!
//! Hyperbolic exposes an OpenAI-compatible `/chat/completions` endpoint. This
//! adapter is a thin wrapper over `OpenAiProvider` that:
//!
//! 1. Namespaces model identifiers with a `hyperbolic/` prefix so they don't
//!    collide with other providers.
//! 2. Strips the `hyperbolic/` prefix from the model name before forwarding.
...
pub struct HyperbolicProvider(OpenAiProvider);
...
impl HyperbolicProvider {
    ...
    fn strip_prefix(model: &str) -> &str {
        model.strip_prefix("hyperbolic/").unwrap_or(model)
    }
}
...
impl LlmProvider for HyperbolicProvider {
    ...
    fn supports_model(&self, model: &str) -> bool {
        model.starts_with("hyperbolic/")
    }

    #[tracing::instrument(
        name = "provider.hyperbolic.chat_completion",
    ...
    #[tracing::instrument(
        name = "provider.hyperbolic.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "hyperbolic",
