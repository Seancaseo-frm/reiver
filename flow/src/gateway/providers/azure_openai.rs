...
//!
//! Azure OpenAI exposes an OpenAI-compatible `/chat/completions` endpoint. This
//! adapter is a thin wrapper over `OpenAiProvider` that:
//!
//! 1. Namespaces model identifiers with a `azure/` prefix so they don't
//!    collide with other providers.
//! 2. Strips the `azure/` prefix from the model name before forwarding.
...
impl LlmProvider for AzureOpenAiProvider {
    ...
    #[tracing::instrument(
        name = "provider.azure_openai.chat_completion",
    ...
    #[tracing::instrument(
        name = "provider.azure_openai.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "azure-openai",
