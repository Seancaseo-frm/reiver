//! AI21 (Jamba) provider adapter.
//!
//! AI21 exposes Jamba models behind an OpenAI-compatible
...
//! | Env var | Description |
//! |---|---|
//! | `GATEWAY_AI21_BASE_URL` | API endpoint (default: `https://api.ai21.com/studio/v1`) |
//! | `GATEWAY_TIMEOUT_AI21_SECONDS` | HTTP timeout (default: 120) |
...
pub struct Ai21Provider(OpenAiProvider);
...
impl Ai21Provider {
    ...
    fn strip_prefix(model: &str) -> &str {
        model.strip_prefix("ai21/").unwrap_or(model)
    }
}
...
impl LlmProvider for Ai21Provider {
    ...
    #[tracing::instrument(
        name = "provider.ai21.chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            message_count = request.messages.len(),
            gen_ai.provider.name = "ai21",
    ...
    #[tracing::instrument(
        name = "provider.ai21.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "ai21",
