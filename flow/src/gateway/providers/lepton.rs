//! Lepton provider adapter.
//!
//! Lepton exposes an OpenAI-compatible `/chat/completions` endpoint. This
//! adapter is a thin wrapper over `OpenAiProvider` that:
//!
//! 1. Namespaces model identifiers with a `lepton/` prefix so they don't
//!    collide with other providers.
//! 2. Strips the `lepton/` prefix from the model name before forwarding.
...
pub struct LeptonProvider(OpenAiProvider);
...
impl LeptonProvider {
    ...
    fn strip_prefix(model: &str) -> &str {
        model.strip_prefix("lepton/").unwrap_or(model)
    }
}
...
impl LlmProvider for LeptonProvider {
    ...
    fn supports_model(&self, model: &str) -> bool {
        model.starts_with("lepton/")
    }

    #[tracing::instrument(
        name = "provider.lepton.chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            message_count = request.messages.len(),
            gen_ai.provider.name = "lepton",
    ...
    #[tracing::instrument(
        name = "provider.lepton.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "lepton",
