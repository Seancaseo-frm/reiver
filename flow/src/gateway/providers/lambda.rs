...
//!
//! Lambda exposes an OpenAI-compatible `/chat/completions` endpoint. This
//! adapter is a thin wrapper over `OpenAiProvider` that:
//!
//! 1. Namespaces model identifiers with a `lambda/` prefix so they don't
//!    collide with other providers.
//! 2. Strips the `lambda/` prefix from the model name before forwarding.
...
pub struct LambdaProvider(OpenAiProvider);
...
impl LambdaProvider {
    ...
    fn strip_prefix(model: &str) -> &str {
        model.strip_prefix("lambda/").unwrap_or(model)
    }
}

#[async_trait]
impl LlmProvider for LambdaProvider {
    fn name(&self) -> Provider {
        Provider::Lambda
    }

    fn supports_model(&self, model: &str) -> bool {
        model.starts_with("lambda/")
    }

    #[tracing::instrument(
        name = "provider.lambda.chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            message_count = request.messages.len(),
            gen_ai.provider.name = "lambda",
    ...
    #[tracing::instrument(
        name = "provider.lambda.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "lambda",
