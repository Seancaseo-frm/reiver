...
//!
//! Cloudflare exposes an OpenAI-compatible `/chat/completions` endpoint. This
//! adapter is a thin wrapper over `OpenAiProvider` that:
//!
//! 1. Namespaces model identifiers with a `cloudflare/` prefix so they don't
//!    collide with other providers.
//! 2. Strips the `cloudflare/` prefix from the model name before forwarding.
...
pub struct CloudflareProvider(OpenAiProvider);
...
impl CloudflareProvider {
    ...
    fn strip_prefix(model: &str) -> &str {
        model.strip_prefix("cloudflare/").unwrap_or(model)
    }
}
...
impl LlmProvider for CloudflareProvider {
    ...
    fn supports_model(&self, model: &str) -> bool {
        model.starts_with("cloudflare/")
    }

    #[tracing::instrument(
        name = "provider.cloudflare.chat_completion",
    ...
    #[tracing::instrument(
        name = "provider.cloudflare.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "cloudflare",
