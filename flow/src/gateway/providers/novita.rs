...
//!
//! Novita exposes an OpenAI-compatible `/chat/completions` endpoint. This
//! adapter is a thin wrapper over `OpenAiProvider` that:
//!
//! 1. Namespaces model identifiers with a `novita/` prefix so they don't
//!    collide with other providers.
//! 2. Strips the `novita/` prefix from the model name before forwarding.
...
pub struct NovitaProvider(OpenAiProvider);
...
impl NovitaProvider {
    ...
    fn strip_prefix(model: &str) -> &str {
        model.strip_prefix("novita/").unwrap_or(model)
    }
}
...
impl LlmProvider for NovitaProvider {
    ...
    fn supports_model(&self, model: &str) -> bool {
        model.starts_with("novita/")
    }

    #[tracing::instrument(
        name = "provider.novita.chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            message_count = request.messages.len(),
            gen_ai.provider.name = "novita",
    ...
    #[tracing::instrument(
        name = "provider.novita.stream_chat_completion",
        skip(self, request, api_key),
        fields(
            model = %request.model,
            gen_ai.provider.name = "novita",
