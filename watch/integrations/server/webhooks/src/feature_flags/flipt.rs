...
//! Accepts Flipt webhook format directly, converts to our format internally
...
pub async fn handler<S: FeatureFlagEventStorage>(
    ...
    let event_type = payload.get("type").and_then(|v| v.as_str()).unwrap_or("flag.updated");
    ...
    let project_key = payload.get("_reiver_project_key")
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| {
            error!("[EVENTS] Missing project_key in Flipt webhook. Add 'reiver_project_key' to webhook metadata.");
            "Missing required field: reiver_project_key (add to Flipt webhook metadata)".to_string()
        })?.to_string();
    
    let enabled = flag_obj.get("enabled").and_then(|v| v.as_bool()).unwrap_or(false);
    let namespace = payload.get("namespace").and_then(|v| v.as_str()).map(|s| s.to_string());
    ...
    let change_event = FeatureFlagChangeEvent {
        ...
        metadata: Some(serde_json::json!({
            "source": "flipt",
            "flipt_event_type": event_type,
            "flipt_webhook": payload.clone(),
        })),
    ...
    info!("[EVENTS] Processed Flipt webhook: flag={}, change_id={}", flag_id, change_id);
