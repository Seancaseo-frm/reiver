...
pub async fn list_incident_exceptions(
    ...
    let lookback = (start_dt - chrono::Duration::days(30)).to_rfc3339();
    let sql = r#"
        ...
        WHERE e.project_id = ?
          AND e.timestamp >= parseDateTime64BestEffort(?)
    ...
    let out: Vec<IncidentExceptionSummary> = rows
        ...
        .map(|r| IncidentExceptionSummary {
            ...
            message: r.message,
            exception_type: Some(r.exception_type),
...
// TODO: The log_templates queries were removed (table was never populated).
// Consider querying reiver.logs directly to provide structured log context
// for incidents (e.g. group by body/severity_text for error pattern aggregation).
...
pub async fn get_incident_context(
    ...
    // Alerts – fired in the window (state_changed_at between start and end)
    ...
    let alerts: Vec<IncidentContextAlert> = sqlx::query_as::<_, AlertRow>(
        r#"
        SELECT a.id, a.rule_id, r.name AS rule_name, a.state,
               a.state_changed_at AS fired_at, a.value
        FROM alerts a
        JOIN alert_rules r ON a.rule_id = r.id
        WHERE r.project_id = $1
          AND a.state_changed_at >= $2 AND a.state_changed_at <= $3
        ...
        let mut logs_around_query = state.clickhouse.as_ref().query(
            if q.service_name.is_some() {
                "SELECT body, severity_text, service_name, log_attributes['source'] AS source, timestamp
                 FROM reiver.logs
                 WHERE project_id = ? AND service_name = ? AND timestamp >= parseDateTime64BestEffort(?) AND timestamp < parseDateTime64BestEffort(?)
                 ORDER BY timestamp DESC LIMIT 100"
            } else {
                "SELECT body, severity_text, service_name, log_attributes['source'] AS source, timestamp
                 FROM reiver.logs
