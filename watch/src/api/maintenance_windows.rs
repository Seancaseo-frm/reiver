...
pub fn create_maintenance_windows_router() -> Router<Arc<WatchState>> {
    Router::new()
...
async fn create_window(
    ...
    let mut audit = AuditEventBuilder::new(AuditEventType::MaintenanceWindowCreated)
        .resource("maintenance_window", id)
        .details(serde_json::json!({ "created": { "name": &req.name, "schedule_type": &req.schedule_type } }))
...
async fn update_window(
    ...
    // Save before-state for audit before values are moved
    ...
    let audit_origin = AuditOrigin::from_headers(&headers);
    let audit_caller = AuditCaller::from_headers(&headers);
    let mut audit = AuditEventBuilder::new(AuditEventType::MaintenanceWindowUpdated)
        .resource("maintenance_window", id)
        .details(serde_json::json!({
            "before": { "name": &before_name, "enabled": before_enabled, "schedule_type": &before_schedule_type },
            "after": { "name": &name, "enabled": enabled, "schedule_type": &schedule_type }
        }))
        .origin(&audit_origin.origin_type, &audit_origin.origin_ref, &audit_origin.origin_reason)
        .caller(&audit_caller.caller_type, &audit_caller.key_label, &audit_caller.key_prefix)
        .success();
...
async fn delete_window(
    ...
    let deleted_row = sqlx::query_as::<_, MaintenanceWindow>(
    ...
    let audit_origin = AuditOrigin::from_headers(&headers);
    let audit_caller = AuditCaller::from_headers(&headers);
    let mut audit = AuditEventBuilder::new(AuditEventType::MaintenanceWindowDeleted)
        .resource("maintenance_window", id)
        .details(serde_json::json!({ "deleted": { "name": deleted_row.as_ref().map(|r| &r.name), "schedule_type": deleted_row.as_ref().map(|r| &r.schedule_type) } }))
        .origin(&audit_origin.origin_type, &audit_origin.origin_ref, &audit_origin.origin_reason)
        .caller(&audit_caller.caller_type, &audit_caller.key_label, &audit_caller.key_prefix)
        .success();
    if let Some(org_id) = organization_id {
