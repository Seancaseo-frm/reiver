...
pub fn create_alert_rules_router() -> Router<Arc<WebsiteState>> {
    Router::new()
...
// ============================================================================
// Types - Simplified HyperDX-style model
// ============================================================================
    ...
    match q {
        ...
        AlertQueryConfig::LogPattern { patterns, .. } => {
            if patterns.is_empty() {
                return Err(AppError::Validation(
                    "log_pattern requires at least one pattern".into(),
                ));
            }
        }
...
async fn list_rules(
    ...
    let scoped_project_id = match authenticate_request_or_api_key(&headers, &state, RateLimitType::Crud).await? {
        ...
        AuthIdentity::ApiKey { project_id: key_pid } => {
            if let Some(project_id) = params.project_id {
                if project_id != key_pid {
                    return Err(AppError::Auth("API key does not belong to this project".into()));
                }
            }
            Some(key_pid)
        }
...
async fn create_rule(
    ...
    let user_id = authenticate_and_verify_project(&headers, &state, &state.db, payload.project_id, RateLimitType::Crud).await?;
    ...
    AuditEventBuilder::new(AuditEventType::AlertRuleCreated)
        .actor(user_id)
        .resource("alert_rule", row.id)
        .details(serde_json::json!({ "created": { "name": &payload.name, "project_id": payload.project_id, "enabled": payload.enabled } }))
...
async fn get_rule(
    ...
    authenticate_and_verify_project(&headers, &state, &state.db, row.project_id, RateLimitType::Crud).await?;
...
async fn update_rule(
    ...
    let user_id = authenticate_and_verify_project(&headers, &state, &state.db, before_row.project_id, RateLimitType::Crud).await?;
    ...
    let audit_origin = AuditOrigin::from_headers(&headers);
    let audit_caller = AuditCaller::from_headers(&headers);
    AuditEventBuilder::new(AuditEventType::AlertRuleUpdated)
        .actor(user_id)
        .resource("alert_rule", id)
        .details(serde_json::json!({
            "before": { "name": &before_row.name, "enabled": before_row.enabled, "project_id": before_row.project_id },
            "after": { "name": &row.name, "enabled": row.enabled, "project_id": row.project_id }
        }))
        .origin(&audit_origin.origin_type, &audit_origin.origin_ref, &audit_origin.origin_reason)
        .caller(&audit_caller.caller_type, &audit_caller.key_label, &audit_caller.key_prefix)
        .success()
...
async fn delete_rule(
    ...
    let user_id = authenticate_and_verify_project(&headers, &state, &state.db, deleted_row.project_id, RateLimitType::Crud).await?;
    ...
    let audit_origin = AuditOrigin::from_headers(&headers);
    let audit_caller = AuditCaller::from_headers(&headers);
    AuditEventBuilder::new(AuditEventType::AlertRuleDeleted)
        .actor(user_id)
        .resource("alert_rule", id)
        .details(serde_json::json!({ "deleted": { "name": &deleted_row.name, "project_id": deleted_row.project_id } }))
        .origin(&audit_origin.origin_type, &audit_origin.origin_ref, &audit_origin.origin_reason)
        .caller(&audit_caller.caller_type, &audit_caller.key_label, &audit_caller.key_prefix)
        .success()
        .log(&state.clickhouse)
...
async fn get_rule_alerts(
    ...
    let project_id: Uuid = sqlx::query_scalar("SELECT project_id FROM alert_rules WHERE id = $1")
        .bind(rule_id)
        .fetch_optional(&*state.db)
        .await
        .map_err(|e| AppError::Internal(anyhow::anyhow!("Database error: {}", e)))?
...
async fn test_notification(
    ...
    authenticate_request_or_api_key(&headers, &state, RateLimitType::Crud).await?;
    ...
    for channel_id in &payload.channel_ids {
        match load_notification_channel(&state.db, *channel_id).await {
            Ok(Some(channel)) => {
                ...
                match send_notification(&channel, &test_notification, state.email.as_deref()).await
                {
                    Ok(_) => {
