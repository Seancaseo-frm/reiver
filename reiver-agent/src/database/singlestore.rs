//! SingleStore (formerly MemSQL) database monitoring collector
//!
//! This collector uses the `sqlx` crate for SingleStore connectivity.
//! SingleStore is MySQL-compatible, so we can use similar queries.
//! For SingleStore performance monitoring, the collector queries:
//! - performance_schema.events_statements_summary_by_digest (MySQL compatibility)
//!
...
pub struct SingleStoreCollector {
    config: Arc<DatabaseConfig>,
    pool: Option<MySqlPool>,
}

impl SingleStoreCollector {
    ...
    pub async fn new(config: Arc<DatabaseConfig>) -> Result<Self> {
        ...
        info!("Connecting to SingleStore database...");
        ...
        let pool = MySqlPoolOptions::new()
            ...
            .connect(&conn_str)
            .await
            .context(format!("Failed to connect to SingleStore database: {}", config.name))?;

        let duration_ms = start.elapsed().as_millis();
        info!(duration_ms, "Connected to SingleStore database");
    ...
    fn query_performance_schema(pool: &MySqlPool, config: &DatabaseConfig) -> Result<Vec<QueryMetricRow>> {
        ...
        if !performance_schema_enabled {
            warn!("performance_schema is not enabled. Please enable it in SingleStore configuration (performance_schema=ON)");
        ...
        // SingleStore is MySQL-compatible, so this works the same way
        let query = format!(
            "SELECT 
                SCHEMA_NAME,
                DIGEST,
        ...
        let mut base_tags = vec![
            ...
            KeyValue::new("db_type", "singlestore"),
        ];
        ...
        for row in rows {
            ...
            let total_time_ms: f64 = row.try_get("total_time_ms")?;
            let mean_time_ms: f64 = row.try_get("mean_time_ms")?;
            let min_time_ms: f64 = row.try_get("min_time_ms")?;
            let max_time_ms: f64 = row.try_get("max_time_ms")?;
...
impl Collector for SingleStoreCollector {
    fn register_observables(&self, meter: opentelemetry::metrics::Meter) -> Result<()> {
        ...
        let _query_calls = meter
            .u64_observable_counter("database.singlestore.queries.calls")
        ...
        let _query_mean_time = meter
            .f64_observable_gauge("database.singlestore.queries.mean_time_ms")
        ...
        let _query_total_time = meter
            .f64_observable_gauge("database.singlestore.queries.total_time_ms")
        ...
        let _query_min_time = meter
            .f64_observable_gauge("database.singlestore.queries.min_time_ms")
        ...
        let _query_max_time = meter
            .f64_observable_gauge("database.singlestore.queries.max_time_ms")
...
/// Normalize query for fingerprinting (MySQL/SingleStore style)
fn normalize_mysql_query(query: &str) -> String {
    ...
    // Replace MySQL parameter markers (?, :1, :2, etc.)
