//! TiDB database monitoring collector
//!
//! This collector uses the `sqlx` crate for TiDB connectivity.
//! TiDB is MySQL-compatible, so we can use similar queries.
//! For TiDB performance monitoring, the collector queries:
//! - performance_schema.events_statements_summary_by_digest (MySQL compatibility)
//! - information_schema.cluster_statements_summary (TiDB-specific, if available)
//!
...
impl TiDBCollector {
    ...
    pub async fn new(config: Arc<DatabaseConfig>) -> Result<Self> {
        ...
        info!("Connecting to TiDB database...");
        ...
        let pool = MySqlPoolOptions::new()
            ...
            .connect(&conn_str)
            .await
            .context(format!("Failed to connect to TiDB database: {}", config.name))?;

        let duration_ms = start.elapsed().as_millis();
        info!(duration_ms, "Connected to TiDB database");
    ...
    fn query_performance_schema(pool: &MySqlPool, config: &DatabaseConfig) -> Result<Vec<QueryMetricRow>> {
        ...
        if !performance_schema_enabled {
            warn!("performance_schema is not enabled. Please enable it in TiDB configuration (performance_schema=ON)");
        ...
        // TiDB is MySQL-compatible, so this works the same way
        let query = format!(
            "SELECT 
                SCHEMA_NAME,
                DIGEST,
...
impl Collector for TiDBCollector {
    fn register_observables(&self, meter: opentelemetry::metrics::Meter) -> Result<()> {
        ...
        let _query_calls = meter
            .u64_observable_counter("database.tidb.queries.calls")
        ...
        let _query_mean_time = meter
            .f64_observable_gauge("database.tidb.queries.mean_time_ms")
        ...
        let _query_total_time = meter
            .f64_observable_gauge("database.tidb.queries.total_time_ms")
        ...
        let _query_min_time = meter
            .f64_observable_gauge("database.tidb.queries.min_time_ms")
        ...
        let _query_max_time = meter
            .f64_observable_gauge("database.tidb.queries.max_time_ms")
...
/// Normalize query for fingerprinting (MySQL/TiDB style)
